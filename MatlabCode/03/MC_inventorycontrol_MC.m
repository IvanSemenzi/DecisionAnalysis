%% File name: MC_inventorycontrol_MC.m
...
for h=1:Nrun
  [x(:,h), u(:,h), gt(:,h)] = IC_singlerun_MC(T_max); % Transition probability matrix
end
...
legend('Histogram',['Mean value = ' num2str(mJ)]);
xlabel('Total cost');
ylabel('Probability density');
title(['Total cost distribution from N=' num2str(Nrun) ' simulation runs (T=' num2str(T_max) ')']);
grid;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
xlabel('Time');
ylabel('Cost');
title(['Average stage cost from N=' num2str(Nrun) ' simulation runs' ]);
grid;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ylabel('Probability');
title(['First order time distribution from N=' num2str(Nrun) ' simulation runs (T=' num2str(T_max) ')']);
grid;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
ylabel('Probability');
title(['Time between orders distribution from N=' num2str(Nrun) ' simulation runs (T=' num2str(T_max) ')']);
grid;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for t=1:T_max+1
  ...
  nX = histcounts(x(t,:),[-0.5:C+0.5],'Normalization','probability');
    if t==1
      figure(5)
      bb=stem(L,nX);
      xlabel('Inventory levels');
      ylabel('Probability');
      tt=title(['State distribution at t=' num2str(t-1) ' from N=' num2str(Nrun) ' simulation runs']); 
      axis([-1 C+1 0 1]);
      grid;
      pause(2)
    else
      figure(5)
      set(bb,'YData',nX);
      set(tt,'String', ['State distribution at t=' num2str(t-1) ' from N=' num2str(Nrun) ' simulation runs']);
